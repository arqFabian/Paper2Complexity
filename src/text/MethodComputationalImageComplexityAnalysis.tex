...
This fundamental concept underpins the CICA system's methodology(Figure\ref{fig:CICAImageEvaluationFlowchart}).
...
The complexity scoring function \(f_1(x)\), as defined in Equation\ref{eq:F1_ComplexityScoreFunction} and detailed in Table\ref{tab:ComplexityScoreFunction}, is a fundamental component of our CICA system.
...
This weighted sum yields the overall complexity score for each building image, as summarized in Table\ref{tab:ComplexityScoreFunction}.
...
The CICA system's first application is the historical analysis of 180 buildings from various architectural eras (Table\ref{tab:CICAPlotMaster}(b)), resulting in a scatter graph demonstrating complexity trends over time.

The second application involves ranking 3D-modeled facades for the VR experiment(Table\ref{tab:CICAPlotMaster}(a)).
Using Blender, we modeled facades with varying complexity, categorized into ten levels based on CICA scores(Table\ref{tab:PatternsVariationsPart0}).
